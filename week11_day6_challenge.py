...
"""Lets analyse a flight safety trend over the time beginning from 1940, assuming that before 1940 the ammount of flights was not sufficient enough. I suggest to use a Survival Rate as an andicator becouse the higher the survival rate, the less serious problems the aircraft faced."""
...
# Calculate survival rate
df['Survival_Rate'] = (df['Aboard'] - (df['Fatalities Passangers'] + df['Fatalities Crew'])) / df['Aboard']
df['Survival_Rate'] = df['Survival_Rate'].clip(0, 1)

# Filter data from 1940 to 2020
df_filtered = df[(df['Year'] >= 1940) & (df['Year'] <= 2020)]
yearly_survival = df_filtered.groupby('Year')['Survival_Rate'].mean().reset_index()
...
plt.tight_layout()
plt.savefig('survival_rate_nonlinear_trend_1940_2020.png')
plt.show()
...
# Calculate overall change in trend
trend_change = trend_2020 - trend_1940
trend_percent_change = (trend_change / trend_1940) * 100

print(f"\nChange in trend from 1940 to 2020: {trend_change:.2%}")
print(f"Percent change in trend: {trend_percent_change:.2f}%")

"""If we consider the Survival Rate as an indicator of flight safety, then from 1940 to 1992, flight safety increased. Starting in 1992, it began to decline, but by 2020, it was still higher than in 1940."""